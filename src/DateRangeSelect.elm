...
import Style exposing (flexHoriz, flexVert, marginLeft10, st)
    ...
    case a of
        RangeMsg msg ->
            ...
            { m
                | range = newRange
                , fromField =
                    List.foldl DateField.update
                        m.fromField
                        [ DateField.SetMax newRange.to
                        , DateField.SetVal newRange.from
                        ]
                , toField =
                    List.foldl DateField.update
                        m.toField
                        [ DateField.SetVal newRange.to
                        , DateField.SetMin newRange.from
                        ]
            }
...
viewField df act str =
    Html.div [ st [ flexVert, marginLeft10 ] ]
        ...
        , Html.map act (DateField.view df)
        ]
