...
decode str =
    List.foldl replace str charMap


config : Config.Config
config =
    Configs.getConfig "de_CH"
...
timeToString time =
    Format.format config "%Y-%m-%d %H:%M:%S" <| Date.fromTime time


dateToString : Date -> String
dateToString dt =
    Format.format config "%Y-%m-%d" dt
...
notBefore a b =
    case a of
        ...
        Just target ->
            if Compare.is Compare.Before b target then
                target
            else
                b
...
notAfter a b =
    case a of
        ...
        Just target ->
            if Compare.is Compare.After b target then
                target
            else
                b
    ...
    case a of
        ...
        Just dateA ->
            case b of
                ...
                Just dateB ->
                    Compare.is Compare.Same dateA dateB
...
quarterOf : Month -> Int
quarterOf mon =
    ceiling ((toFloat <| monthToInt mon) / 3)
...
isBefore when what =
    case when of
        ...
        Just than ->
            Compare.is Compare.Before than what
...
isAfter when what =
    case when of
        ...
        Just than ->
            Compare.is Compare.After than what
