...
type alias StyleDef = List(String,String)

boxSize = 25
boxPx i = (toString (i * boxSize) ++ "px")

st: List StyleDef->Html.Attribute a
st l = HA.style (List.concatMap identity l)

border: String->StyleDef
border clr = ["border"=> ("1px solid " ++ clr)]
...
headerMonthCellStyle month count = 
  let clr = if month % 2 == 0 then "#f1ae87" else "#ffe3d3"
  in headerGridCellStyle ++ 
     [ "backgroundColor" => clr
     , "flex" => "auto"
     , "overflow" => "hidden"
...
headerWeekCellStyle wk = 
  let clr = if wk % 2 == 0 then "#a9d190" else "#dfeddb"
  in headerBoxCellStyle ++ 
     [ "backgroundColor" => clr ]
